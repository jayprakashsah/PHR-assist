...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "User registered successfully!", id: newUser._id, name: newUser.name, email: newUser.email });
  } catch (error) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Login successful!", id: user._id, name: user.name, email: user.email });
  } catch (error) {

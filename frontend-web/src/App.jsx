...
import './App.css'; // We will use the default Vite CSS file for basic styling
...
function App() {
  ...
  // 1. Send the image to the Python AI Server
  ...
  // 2. Save the AI results to our Node.js Database
  const handleSaveToDB = async () => {
    try {
      // Calling our Node.js main backend
      await axios.post("http://localhost:5000/api/reports/add", reportData);
      setIsSaved(true);
      alert("Report securely saved to your health profile!");
    } catch (error) {
      console.error("Error saving report:", error);
      alert("Failed to save. Is the Node.js server running?");
    }
  };
